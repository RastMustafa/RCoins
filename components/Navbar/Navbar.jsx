...
function Navbar() {
  ...
  return (
    <nav className="flex font-header tracking-widest		 relative h-20 justify-center z-20  border-b-2 border-black    container mx-auto px-5 py-5">
      {/*========================== website logo START ==========================*/}
      <div className="w-24  rounded-full -z-10  top-4 mr-2  left-0 absolute">
        <Link href="/">
          <div className="text-4xl ml-2 mt-2">
            <span className="text-yellow-500  text-5xl     inline-block h-10">R</span>
            <span>C</span>
            <span>O</span>
            <span>I</span>
            <span>N</span>{" "}
      ...
      <ul className="hidden lg:flex my-auto   items-center h-8 space-x-5 ">
        {catagories.map((item, i) => {
          return (
            <LazyNavbarDropDownMenu key={`${item}_${i}`} explorTabData={catagories[i]} title={navbarTabTitles[i]} />
          );
        })}
      </ul>
      ...
      <div className="hidden  lg:block top-6 w-20  mr-12  right-0 absolute">
        <button className="border-2 font-display border-black shadow rounded font-semibold   hover:bg-black transition-all duration-200  hover:text-white px-8 text-lg">
          Login
        </button>
      </div>
      {/******************************** login Btn END ********************************/}
      ...
      <div className="flex lg:hidden  bg-red-400">
        <a href="#" className="flex-none absolute right-0 px-5 z-30">
          ...
          >
            {/* dropdown logo start */}
            <span className=" block w-6 border-b-2  border-black "></span>
            <span className=" block  my-1 w-6 border-b-2 border-black "></span>
            <span className=" block  w-4 border-b-2 border-black "></span>
            {/* dropdown logo end */}
          </div>
        </a>
        <div className="w-full">
          {showMenu ? <DropDownMobileSize navbarTabTitles={navbarTabTitles} icon={icon} /> : ""}
        </div>
      </div>
